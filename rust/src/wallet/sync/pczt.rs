...
use super::{
    consume_stored_proposal, discard_names_transaction_capability, discard_stored_proposal,
    finish_stored_proposal, open_wallet_db, retain_names_transaction_capability_until_expiry,
...
pub fn discard_proposal(proposal_id: u64, send_flow_id: &str) -> Result<(), String> {
    if discard_names_transaction_capability(proposal_id, send_flow_id)? {
        return Ok(());
    }
...
pub fn retain_proposal_lock_until_expiry(
    ...
    if retain_names_transaction_capability_until_expiry(proposal_id, send_flow_id)? {
        return Ok(());
    }
